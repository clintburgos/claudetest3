...
pub fn test_camera_zoom_issue() {
    ...
    println!("  Map: {}x{} tiles ({:.0}x{:.0} pixels)", 
    ...
    for (zoom, description) in &zoom_levels {
        ...
        println!("  Visible area: {:.0}x{:.0} pixels", visible_width, visible_height);
        println!("  Visible tiles: {:.1}x{:.1}", 
            visible_width / tile_size, visible_height / tile_size);
        ...
        if visible_width >= map_width_pixels && visible_height >= map_height_pixels {
            ...
            println!("  Empty space: {:.0}x{:.0} pixels", empty_space_x, empty_space_y);
        }
