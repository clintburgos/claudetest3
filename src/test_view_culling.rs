...
pub fn test_view_culling_debug(_commands: Commands) {
    ...
    let test_cases = vec![
        ...
        (Vec2::new(0.25, 0.25), 1280.0, 720.0, "Very zoomed in (0.25x)"),
    ];

    println!("\n=== VIEW CULLING DEBUG TEST ===\n");

    for (scale, width, height, description) in test_cases {
        println!("Test case: {} - Scale: {:?}, Viewport: {}x{}", description, scale, width, height);
...
fn debug_visible_bounds(camera_pos: Vec2, camera_scale: Vec2, viewport_width: f32, viewport_height: f32) {
    ...
    println!("      half_width = (viewport_width / 2) * scale.x = ({} / 2) * {} = {}", 
        viewport_width, camera_scale.x, half_width);
    println!("      half_height = (viewport_height / 2) * scale.y = ({} / 2) * {} = {}", 
    ...
    println!("      min_x = camera.x - half_width = {} - {} = {}", camera_pos.x, half_width, min_x);
    println!("      max_x = camera.x + half_width = {} + {} = {}", camera_pos.x, half_width, max_x);
    println!("      min_y = camera.y - half_height = {} - {} = {}", camera_pos.y, half_height, min_y);
    println!("      max_y = camera.y + half_height = {} + {} = {}", camera_pos.y, half_height, max_y);
    ...
    println!("      area = {} * {} = {}", visible_width, visible_height, visible_area);
    ...
    println!("      X: {} to {} ({} tiles)", min_tile_x, max_tile_x, max_tile_x - min_tile_x);
    println!("      Y: {} to {} ({} tiles)", min_tile_y, max_tile_y, max_tile_y - min_tile_y);
    println!("      Total tiles in view: {}", (max_tile_x - min_tile_x) * (max_tile_y - min_tile_y));
}
