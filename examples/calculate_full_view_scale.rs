...
fn main() {
    ...
    println!("  Bottom-left (0,0): ({:.1}, {:.1})", bottom_left.x, bottom_left.y);
    println!("  Top-right ({},{})

: ({:.1}, {:.1})", grid_width-1, grid_height-1, top_right.x, top_right.y);
    println!("  Bottom-right: ({:.1}, {:.1})", bottom_right.x, bottom_right.y);
    ...
    println!("  Can see entire map: {}", if visible_at_min >= world_width { "YES" } else { "NO" });
}