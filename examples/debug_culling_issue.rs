...
fn main() {
    ...
    println!("Grid: {}x{}, tile_size: {}", grid_config.width, grid_config.height, grid_config.tile_size);
    ...
    for scale in scales {
        ...
        let center = grid_to_world(grid_config.width / 2, grid_config.height / 2, 0, grid_config.tile_size);
        ...
        println!("Visible area: {:.1} x {:.1} world units", visible_width, visible_height);
        ...
        println!("Raw bounds: ({}, {}) to ({}, {})", min_x, min_y, max_x, max_y);
        ...
        println!("Clamped bounds: ({}, {}) to ({}, {})", min_x, min_y, max_x, max_y);
        println!("Visible tiles: {}x{} = {} tiles", max_x - min_x + 1, max_y - min_y + 1, (max_x - min_x + 1) * (max_y - min_y + 1));
        
        // Check if entire map should be visible
        if min_x == 0 && max_x == grid_config.width - 1 && min_y == 0 && max_y == grid_config.height - 1 {
